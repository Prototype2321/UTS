...
<body class="bg-gray-100">
    ...
    <form method="post" action="logout.php">
                <button type="submit" class="bg-red-500 text-white font-bold py-2 px-4 rounded hover:bg-red-600 ml-4">Logout</button>
            </form>
        </div>
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            // echo '<input type="hidden" name="task_completed" value="' . $row["id"] . '">';
            // echo '<button type="submit" class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">';
            // echo 'Mark as ' . ($row["is_completed"] ? "Not Completed" : "Completed");
            // echo '</button>';
            // echo '</form>';
            echo '<div class="mt-4">';
            ...
            // echo '<a href="progress.php?id=' . $row["id"] . '" class="text-green-500 hover:underline">Save</a>';
            echo '</div>';
            echo "</div>";
